...
// 상속 : 개게지향 프로그래밍 특성 중 하나
// 원래 존재하던 클래스를 '확장하여 재사용할 수 있ㄷ록 하는 것
...
class Developer extends Human{
	...
	Developer(String name, int age, String[] skills){
		...
		super(name, age);
		this.skills = skills;
		// 부모 생성자의 호출은 반드시 현재 생성자의 첫번째 구문으로 작성해야함
	}
	
	
	void develop() {
		System.out.println(Arrays.toString(skills) + "로 개발을 합니다. ");
	}
}
...
class SalesMan extends Human {
	...
	// 규칙
	// 1. 상위 클래스에서 선언된 메서드와 메서드 이름, 매개변수의 조합이 같아야 됨
	// 2. 상위 클래스에서 선언된 메서드가 final로 선언되면 재정의가 불가능
	
	//final 로 선언된 메서드는 오버라이딩 할 수 없음
	@Override
	void sayHello() {
		super.sayHello();
...
// Java에서는 단일 상속만 지원합니다.
// 다중 상속에서 발생할 수 있는 다이아몬드 문제가 발생하지 않도록 하기 위함
// 다중 상속과 동일한 효과를 보기 위해선 인터페이스 혹은 중첩 상속을 수행합니다.
public class Inheritance {
	...
	public static void main(String[] args) {
		...
		salesMan1.equals(""); // Object에 존재하는 메서드
	}
